...
##
# Prints plots of memory as an ASCII-art diagram

class MemInspect::ASCIIViewer < MemInspect::Viewer

  ##
  # Draws an ASCII-art picture and saves it to memory_map.PID.timestamp.txt
  ...
  def draw
    ...
    File.open "memory_map.#{$$}.#{Time.now.to_i}.txt", 'wb' do |fp|
      fp.write canvas.to_s
    end
  end
